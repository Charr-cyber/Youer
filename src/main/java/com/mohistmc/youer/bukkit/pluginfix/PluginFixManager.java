...
import org.bukkit.entity.TeleportFlag;
...
public class PluginFixManager {
    ...
    /**
     * Sync / Async teleport yapar.
     * Paper API varsa async, yoksa sync + chunk load
     */
    public static void teleportForce(Entity entity, Location location) {
        if (entity == null || location == null) return;

        Plugin mythicPlugin = Bukkit.getPluginManager().getPlugin("MythicDungeons");
        if (mythicPlugin == null) return;

        Bukkit.getScheduler().runTask(mythicPlugin, () -> {
            try {
                if (Bukkit.getPluginManager().isPluginEnabled("Paper")) {
    ...
    // ASM patch: forceTeleport2 çağrılarını yakalar
    public static void patchForceTeleport(ClassNode node) {
        for (MethodNode method : node.methods) {
            for (AbstractInsnNode insn : method.instructions) {
                if (insn instanceof MethodInsnNode mInsn) {
                    if (mInsn.owner.equals("net/playavalon/mythicdungeons/utility/helpers/Util")
                            && mInsn.name.equals("forceTeleport2")
                            && mInsn.desc.equals("(Lorg/bukkit/entity/Entity;Lorg/bukkit/Location;)V")) {
                        mInsn.setOpcode(Opcodes.INVOKESTATIC);
                        mInsn.owner = Type.getInternalName(PluginFixManager.class);
                        mInsn.name = "teleportForce";
                        mInsn.desc = "(Lorg/bukkit/entity/Entity;Lorg/bukkit/Location;)V";
                    }
                }
            }
        }
    ...
    public static byte[] injectPluginFix(String plugin, String className, byte[] clazz) {
        ...
        if (className.startsWith("net.Zrips.CMILib.") || className.startsWith("com.Zrips.CMI.")) {
            return patch(clazz, node -> helloWorld(node,
        ...
        Consumer<ClassNode> patcher = switch (className) {
            ...
            case "net.playavalon.mythicdungeons.utility.helpers.Util" -> PluginFixManager::patchForceTeleport; // MythicDungeons patch
            default -> null;
        };
        ...
        node.methods.forEach(method -> {
            for (AbstractInsnNode next : method.instructions) {
                if (next instanceof LdcInsnNode ldcInsnNode) {
                    if (ldcInsnNode.cst instanceof String str && a.equals(str)) {
                        ldcInsnNode.cst = b;
                    }
                }
            }
    ...
    private static void helloWorld(ClassNode node, int a, int b) {
        node.methods.forEach(method -> {
            for (AbstractInsnNode next : method.instructions) {
                if (next instanceof IntInsnNode ldcInsnNode && ldcInsnNode.operand == a) {
